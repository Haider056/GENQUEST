...
app = Flask(__name__)
CORS(app, resources={r"/generate_questions": {"origins": "http://localhost:3000"}})
...
class QuestionGenerator:
    ...
    def __init__(self) -> None:
        ...
        QG_DIR = "iarfmoose/t5-base-question-generator"
        ...
        self.qg_tokenizer = AutoTokenizer.from_pretrained(QG_DIR, use_fast=False)
        self.qg_model = AutoModelForSeq2SeqLM.from_pretrained(QG_DIR)
...
class QAEvaluator:
    ...
    def __init__(self) -> None:
        ...
        QAE_DIR = "iarfmoose/bert-base-cased-qa-evaluator"
        self.SEQ_LENGTH = 512

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        self.qae_tokenizer = AutoTokenizer.from_pretrained(QAE_DIR)
        self.qae_model = AutoModelForSequenceClassification.from_pretrained(QAE_DIR)
...
def generate_questions():
 if request.method == 'OPTIONS':
    ...
    text = data.get('text')  # Use .get() method to safely access dictionary keys
    
    if not text:
        logging.error("No 'text' field found in the request data")
    ...
    try:
        ...
        logging.exception(f"An error occurred while generating questions: {str(e)}")
